...
def auto_insert_images_to_excel(excel_path, sheet_name):
...
if uploaded_file and st.button("开始处理"):
    ...
    st.info("⏳ 正在处理，请稍候...")
    output_path, success_count, fail_count = auto_insert_images_to_excel(os.path.join("temp_images", uploaded_file.name), sheet_name)
